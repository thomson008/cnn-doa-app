...
class DoaApp:
    ...
    def __init__(self, top, tpu=False):
        ...
        self.start_button = Button(self.data_frame, text="Start", command=self.toggle_prediction,
                                   height=2, width=15, font=("Arial", 10), cursor="hand2")
        self.start_button.place(relx=0.5, y=300, anchor=CENTER)
        self.mode_selection = Button(self.data_frame, text="Change mode...", command=self.select_mode,
                                     height=2, width=15, font=("Arial", 10), cursor="hand2")
        self.mode_selection.place(relx=0.5, y=350, anchor=CENTER)

        self.exit_button = Button(self.data_frame, text="Exit", command=self.top.destroy,
                                  height=2, width=15, font=("Arial", 10), cursor="hand2")
        self.exit_button.place(relx=0.5, y=400, anchor=CENTER)
    ...
    def create_frames(self):
        left_frame = LabelFrame(self.top, width=self.HEIGHT, height=self.HEIGHT)
        left_frame.pack(side=LEFT)
        left_frame.pack_propagate(False)

        right_frame = LabelFrame(self.top, width=self.WIDTH - self.DIM - 1, height=self.HEIGHT)
    ...
    def create_title_label(self):
        ...
        label.config(font=("Arial", 26), fg="#4a4a4a")
        label.pack()
